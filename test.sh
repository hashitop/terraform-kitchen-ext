...
export_aws_sts_session "ap-southeast-1"

# Destroy any existing Terraform state in us-east-1
bundle exec kitchen destroy centos diagnose --all
...
bundle exec kitchen converge centos diagnose --all

# Test the Terraform state using the InSpec controls
bundle exec kitchen verify centos diagnose --all

# Destroy the Terraform state using the Terraform fixture configuration
bundle exec kitchen destroy centos diagnose --all

export_aws_sts_session "ap-southeast-2"

# Perform the same steps for Ubuntu in us-west-2
bundle exec kitchen test ubuntu diagnose --all
